...
describe("QuickJsWasmRuntime (browser)", () => {
  it("evaluates code snippets", async () => {
    ...
    const resultJson = runtime.evalUtf8("1 + 3");
    if (resultJson !== "4") {
  ...
  it("keeps global state between evaluations", async () => {
    ...
    const resultJson = runtime.evalUtf8("globalThis.answer + 2");
    if (resultJson !== "42") {
  ...
  it("invokes named functions via callFunctionUtf8", async () => {
    ...
    const resultJson = runtime.callFunctionUtf8("increment", JSON.stringify([4]));
    if (resultJson !== "5") {
