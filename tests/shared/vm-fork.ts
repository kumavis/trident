...
export const vmForkTestCases: VmTestCase[] = [
  {
    ...
    async run() {
      await usingVm(() => createForkableVm(), async (parentVm) => {
        await parentVm.eval("globalThis.x = 1; 0;");
        const childVm = await parentVm.fork();
        try {
          await parentVm.eval("globalThis.x = globalThis.x + 1; 0;");
          await childVm.eval("globalThis.x = globalThis.x + 10; 0;");
          const parentValue = await parentVm.eval("globalThis.x");
          const childValue = await childVm.eval("globalThis.x");
  ...
  },
  {
    ...
    async run() {
      await usingVm(() => createForkableVm(), async (vm) => {
        await vm.eval("globalThis.state = { count: 5 }; 0;");
        const childA = await vm.fork();
        const childB = await vm.fork();
        try {
          await childA.eval("globalThis.state.count += 1; 0;");
          await childB.eval("globalThis.state.count += 10; 0;");
          const aValue = await childA.eval("globalThis.state.count");
          const bValue = await childB.eval("globalThis.state.count");
          const parentValue = await vm.eval("globalThis.state.count");
  ...
  },
  {
    ...
    async run() {
      await usingVm(() => createForkableVm(), async (vm) => {
        await vm.eval("globalThis.payload = { nested: { value: 7 } }; 0;");
        const child = await vm.fork();
        try {
          const childValue = await child.eval("globalThis.payload.nested.value");
          ensureNumber(childValue, 7, "child reads nested value");
          await child.eval("globalThis.payload.nested.value = 100; 0;");
          const parentValue = await vm.eval("globalThis.payload.nested.value");
          ensureNumber(parentValue, 7, "parent remains unchanged");
          const childUpdated = await child.eval("globalThis.payload.nested.value");
